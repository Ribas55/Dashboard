"""
Module for Linear Regression forecasting.
"""
...
def calculate_linear_regression(series: pd.Series, n_lags: int, forecast_horizon: pd.PeriodIndex) -> pd.Series:
    """
    Calculates iterative Linear Regression forecasts using lagged features.
    ...
    for target_period in forecast_horizon:
        ...
        if len(training_data) >= min_obs_needed:
            try:
                ...
                print(f"Warning: Linear Regression failed for period {target_period}. Error: {e}")
                forecasts[target_period] = pd.NA
        else:
            # Not enough data
            forecasts[target_period] = pd.NA

    return pd.Series(forecasts, index=forecast_horizon, name="linear_regression_forecast") 